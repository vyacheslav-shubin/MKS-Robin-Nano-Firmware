...
inline void get_serial_commands() {
  ...
  // send "wait" to indicate Marlin is still waiting.
    ...
    if (commands_in_queue == 0 && !MYSERIAL.available() && ELAPSED(ms, last_command_time + NO_TIMEOUTS)) {
	  ...
	  /*5s?��?2a2?|��?????��2��oy?Y?���㡧��??a??��?D����a?��2?????��??��|��o1��??wifi*/
	 ...
	 ui_app.idle();
        ...
        if (!sd_comment_mode) 
        {
			...
			if(sd_char == 0x5a ) //"Z"
			{
				...
				#if 1
				mksReprint.sdpos_bak = mksReprint.sdpos;//sdpos_bak��?��Y??��?��????�䨺1��?
				if(gCfgItems.pwroff_save_mode != 1)//sd?�������???��?��y?Y
                #endif
...
inline void gcode_G0_G1(
  ...
  if (IsRunning() && G0_G1_CONDITION) {
   ...
   /*    
    ...
    {
        if ((destination[2] <= gCfgItems.breakpoint_z_pos)&&(card.sdpos >= 5000)) //�ϵ�������Ҫ����5k֮������ݲ��ܽ�������
        {
            return;
        }
    }
