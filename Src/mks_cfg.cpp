...
volatile u32 ugbksize;//flash��UnicodeתGBKת�����Сֵ��
...
void mksCardTest()
{
	...
	FIL pft;
	char *mks_pft_name = "mks_pft.sys";	//8.3 �ļ����ṹ
...
void CardReader::mksConfiguration() 
{
	...
	if (f_open(&curFile, (const TCHAR *)oldname, FA_OPEN_EXISTING | FA_READ) == FR_OK)
	{
	 ...
	 mksReset();
	 (void)settings.reset();	//MarlinĬ�ϲ���ֵ�ָ�
	  ...
	  while(1)
	  {
		...
		#if tan_mask//����ָ������Ҫ�ո�ġ�
		//ȥ���ո�
		if(cfg_data != 0x20 && cfg_data != 0x09)	//0x20 -- �ո� 0x09--ˮƽ�Ʊ��
		#endif
		{
			if(notes_flag == 0)
	  			*p++ = (uint8_t)cfg_data;
		...
		if((cfg_data == 0x0A || cfg_data == 0x0D) && notes_flag == 1)  // ���� ,�س�
		{
				notes_flag = 0;
				*p++ = (uint8_t)cfg_data;					
		}
	  ...
	  if(!mksTmp.cfg_hardware_test_enable)  //�����ļ���
	  	{
		  r = f_unlink(newname);
...
void flashBurnValue(char *string,uint16_t flash_addr)
{
	...
	if(tmp_index)
	{
		...
		SPI_FLASH_BufferWrite((u8 *)cmd_code, flash_addr,strlen(cmd_code)+1);//��1��֤���һ���ֽ�Ϊ0
	}
}
...
void hexBurnValue2(char *string,uint8_t *data_addr,uint16_t flash_addr)
{
	...
	if(tmp_index)
	{
		...
		*data_addr = ascii2dec(&cmd_code[2],2);
		...
		SPI_FLASH_BufferWrite((u8 *)data_addr, flash_addr,1);//��1��֤���һ���ֽ�Ϊ0
...
/*------------------------�ж��Ƿ����EEPROM---------------------------------*/
/*
>ERASE_EEPROM 1		#1������ǰ�Ȳ���EEPROM; 0������ǰ������EEPROM;
*/
...
//FILAMENT_CHANGE_X_POS
    ...
    eprBurnValue(">BED_HOME_Z", &mksCfg.bed_homind_z, EPR_BED_HOMING);
...
//CUSTOMER CMD
	...
	eprBurnValue(">MULTIPLE_PROBING", &mksCfg.multiple_probing, EPR_MULTILPE_PROBING);
...
/*--mks cfg XYZ_CONSTS_FROM_CONFIG END--*/
	...
	if(MACHINETPYE == DELTA)
		{
		if(mksCfg.grid_max_points_x%2 == 0) mksCfg.grid_max_points_x -= 1;//ֻ��Ϊ����
		if(mksCfg.grid_max_points_y%2 == 0) mksCfg.grid_max_points_y -= 1;
		if(mksCfg.grid_max_points_x <3)	mksCfg.grid_max_points_x = 3;	// ��С��3��
    ...
    TIM3->CCR3 = mksCfg.current_vref_xy < 1000 ? mksCfg.current_vref_xy*0.364:364;      //A4988,RS=0.2ŷ��XY����������Ϊ1A.
    
    epr_read_data((int)EPR_CURRENT_VREF_Z, (uint8_t*)&mksCfg.current_vref_z, sizeof(mksCfg.current_vref_z)); 
    TIM3->CCR2 = mksCfg.current_vref_z < 1000 ? mksCfg.current_vref_z*0.364:364;      //A4988,RS=0.2ŷ��Z����������Ϊ1A.
	
    epr_read_data((int)EPR_CURRENT_VREF_E, (uint8_t*)&mksCfg.current_vref_e, sizeof(mksCfg.current_vref_e)); 
    TIM3->CCR1 = mksCfg.current_vref_e < 1000 ? mksCfg.current_vref_e*0.364:364;      //A4988,RS=0.2ŷ��E����������Ϊ1A.
#endif
    ...
    epr_read_data((int)EPR_BED_HOMING, (uint8_t*)&mksCfg.bed_homind_z, sizeof(mksCfg.bed_homind_z));
...
//THERMAL_PROTECTION_BED_PERIOD
    ...
    AT24CXX_Read((uint16_t) EPR_MULTILPE_PROBING, &mksCfg.multiple_probing, 1);
