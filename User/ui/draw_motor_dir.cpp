    		...
    		if(pMsg->Data.v == WM_NOTIFICATION_RELEASED)
    		{
    			...
    			else if(pMsg->hWinSrc == XMotorDir.btnHandle)
    			{
					if(mksCfg.invert_x_dir==1)
					{
						...
						BUTTON_SetBmpFileName(XMotorDir.btnHandle, "bmp_disable.bin",1);//����
						if(gCfgItems.multiple_language != 0)
							BUTTON_SetText(XMotorDir.btnHandle, machine_menu.Invert_0);//����
					}
					else
					{
						mksCfg.invert_x_dir=1;
						BUTTON_SetBmpFileName(XMotorDir.btnHandle, "bmp_enable.bin",1);//����
						if(gCfgItems.multiple_language != 0)
							BUTTON_SetText(XMotorDir.btnHandle, machine_menu.Invert_1);//����
					}
